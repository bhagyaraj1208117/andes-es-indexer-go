...
func (aa *alteredAccounts) Add(key string, account *AlteredAccount) {
	...
	isTokenOperation := account.IsDCTOperation || account.IsNFTOperation
	if !isTokenOperation {
	...
	for _, elem := range aa.altered[key] {
		newElementIsTokenOperation := account.IsDCTOperation || account.IsNFTOperation
		oldElementIsTokenOperation := elem.IsDCTOperation || elem.IsNFTOperation
		...
		if shouldRewrite {
			...
			elem.IsDCTOperation = account.IsDCTOperation
