...
import (
	...
	indexerData "github.com/bhagyaraj1208117/andes-es-indexer-go/data"
	indexer "github.com/bhagyaraj1208117/andes-es-indexer-go/process/dataindexer"
	"github.com/bhagyaraj1208117/andes-es-indexer-go/tools/accounts-balance-checker/pkg/utils"
...
// CheckMOAXBalances will compare the MOAX balance from the Elasticsearch database with the results from gateway
func (bc *balanceChecker) CheckMOAXBalances() error {
	return bc.esClient.DoScrollRequestAllDocuments(
		accountsIndex,
		[]byte(matchAllQuery),
		bc.handlerFuncScrollAccountMOAX,
	)
}

var countCheck = 0

func (bc *balanceChecker) handlerFuncScrollAccountMOAX(responseBytes []byte) error {
